...
**돈은 정수로 기록합니다.** 금액과 포인트는 최소 단위 정수로 저장하고, 환불은 기존 기록을 지우지 않고 반대 부호의 항목을 추가합니다.

**중복은 저장소가 막게 합니다.** 웹훅 사전 조회만으로는 동시 유입을 막지 못하므로 멱등 키에 unique 제약을 둡니다.

**서비스 경계와 배포 경계는 다릅니다.** 책임은 분리하되, 트래픽과 장애 격리가 요구하지 않는 파드와 커넥션 풀은 늘리지 않습니다.

**운영과 인계도 기능입니다.** 배포 대상, 롤백, 로그, 환경 계약을 저장소에 남겨 다른 사람이 이어서 운영할 수 있게 합니다.
